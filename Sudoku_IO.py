...
def displayPuzzle(snapshot, screen):
    ...
    for row in range(9):
        for column in range(9):
            pygame.draw.rect(screen, color, [(margin + width) * column + margin + (margin * (column // 3)),
                                             (margin + height) * row + margin + (margin * (row // 3)), width, height])
            ...
            screen.blit(label, ((margin + width) * column + margin + (margin * (column // 3)) + 25,
                                (margin + height) * row + margin + (margin * (row // 3)) + 10))
